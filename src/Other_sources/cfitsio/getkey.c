...
int ffghpr(fitsfile *fptr,  /* I - FITS file pointer                        */
           ...
           int *fitsio_extend,     /* O - may FITS file haave extensions?          */
           int *status)     /* IO - error status                            */
/*
...
*/
{
    ...
    ffgphd(fptr, maxdim, simple, bitpix, naxis, naxes, pcount, gcount, fitsio_extend,
    ...
    *nspace = 0;
    ...
    for (; !found_end; nextkey++)  
    {
      ...
      else /* got the next keyword without error */
      {
        ...
        else if (!strcmp(name, "EXTEND") && fitsio_extend)
        {
            *nspace = 0;  /* reset count of blank keywords */
            ffpsvc(card, value, comm, status); /* parse value and comment */

            if (ffc2ll(value, fitsio_extend, status) > 0) /* convert to logical */
            {
                /* reset error status and continue, but still issue warning */
                *status = tstatus;
                *fitsio_extend = 0;
