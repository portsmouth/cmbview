...

{
  ...
  */
  ...
  for(index_local = 1, offset = 0, i = 1; i <= *ngroups && *status == 0; ++index_local)
    {	  
      sprintf(keyword,"GRPID%d",index_local);
	  ...
	  if(offset > 0)
	    {
	      ...
	      sprintf(keyword,"GRPLC%d",index_local);
	  ...
	  if(iomode == READWRITE)
		...
		identify it as belonging to the group
	      */
	      ...
	      for(index_local = 1, found = 0; index_local <= ngroups && *status == 0 && 
		    !found; ++index_local)
		{	  
		  /* read the next GRPIDn keyword in the series */
  ...
  do
      ...
      if(*status == KEY_NO_EXIST) 
	{
      ...
      *status = fits_get_num_groups(mfptr,&ngroups,status);
      ...
      for(index_local = 1; index_local <= ngroups && *status == 0; ++index_local)
	{
	  /* open the (index)th group linked to the member HDU */ 

	  *status = fits_open_group(mfptr,index_local,&gfptr,status);
      ...
      if(rmopt != 0)
	{
	  ...
	  for(index_local = 1; index_local <= ngroups && *status == 0; ++index_local)
	    {
	      sprintf(keyword,"GRPID%d",(int)index_local);
	      fits_delete_key(mfptr,keyword,status);
	      
	      sprintf(keyword,"GRPLC%d",(int)index_local);
	      fits_delete_key(mfptr,keyword,status);
